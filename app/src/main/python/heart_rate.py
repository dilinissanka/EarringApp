...
class heart_rate():
    ...
    def approx_peak(self):
        ...
        slopes = np.convolve(self.m_win, np.full((25,), 1) / 25, mode='same')
...
########################################################################################################
#The below is for graphing#
########################################################################################################
def main():
    # @title Choose ECG Signal { form-width: "20%", display-mode: "both" }
    signal_number = 9  # @param {type:"slider", min:0, max:9, step:1}
    filename = f'mit-bih-arrhythmia-database-1.0.0/{str(100 + signal_number)}'

    data = np.loadtxt('/storage/emulated/0/henlo/records1.txt')

    QRS_detector = Pan_Tompkins_QRS()
    ecg = pd.DataFrame(np.array([list(range(len(data))),data]).T,columns=['TimeStamp','ecg'])

    output_singal = QRS_detector.solve(ecg, 360)

    # The below code is to get the heart rate
    # Convert ecg signal to numpy array
    signal = ecg.iloc[:, 1].to_numpy()

    # Find the R peak locations
    hr = heart_rate(signal, 360)
    ...
    heartRate = (60 * 360) / np.average(np.diff(result[1:]))
    return heartRate
